...
const MainEstadosExpedientes = ({ className }) => {
    ...
    const mutationAdd = useMutation({
        mutationFn: (newEstado) =>
            fetch('http://34.175.60.182:9000/api/estadosexpedientes', {
                method: 'POST',
                headers: {
    ...
    const mutationUpdate = useMutation({
        mutationFn: (updatedEstado) =>
            fetch(`http://34.175.60.182:9000/api/estadosexpedientes/${updatedEstado.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const mutationDelete = useMutation({
        mutationFn: (id) =>
            fetch(`http://34.175.60.182:9000/api/estadosexpedientes/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['estadosExpedientes'] });
