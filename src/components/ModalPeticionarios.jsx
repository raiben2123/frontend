...
const ModalPeticionarios = ({ isOpen, onClose, onAdd, onUpdate, peticionario, empresas = [] }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (formData.empresaCIF.trim()) {
                const response = await fetch(`http://143.131.204.234:9000/api/empresas/cif/${formData.empresaCIF}`);
    ...
    const handleAddOrUpdateEmpresa = async (empresaData) => {
        try {
            const response = await fetch('http://143.131.204.234:9000/api/empresas', {
                method: 'POST',
                headers: {
