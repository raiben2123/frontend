...
const MainExpedientesPrincipales = ({ className }) => {
    ...
    const mutationSaveExpediente = useMutation({
        mutationFn: async (data) => {
            ...
            const url = data.id ? `http://34.175.60.182:9000/api/expedientesprincipales/${data.id}` : 'http://localhost:8081/api/expedientesprincipales';
    ...
    const mutationDeleteExpediente = useMutation({
        mutationFn: (id) =>
            fetch(`http://34.175.60.182:9000/api/expedientesprincipales/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['expedientes'] });
