...
const MainExpedientesPrincipales = ({ className }) => {
    ...
    const mutationSaveExpediente = useMutation({
        mutationFn: async (data) => {
            ...
            const url = data.id ? `http://localhost:8000/api/expedientesprincipales/${data.id}` : 'http://localhost:8081/api/expedientesprincipales';
    ...
    const mutationDeleteExpediente = useMutation({
        mutationFn: (id) =>
            fetch(`http://localhost:8000/api/expedientesprincipales/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['expedientes'] });
