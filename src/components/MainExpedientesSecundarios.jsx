...
const MainExpedientesSecundarios = ({ className }) => {
    ...
    const mutationSaveExpediente = useMutation({
        mutationFn: async (data) => {
            ...
            const url = data.id ? `http://34.175.60.182:9000/api/expedientessecundarios/${data.id}` : 'http://34.175.60.182:9000/api/expedientessecundarios';
    ...
    const mutationDeleteExpediente = useMutation({
        mutationFn: (id) =>
            fetch(`http://34.175.60.182:9000/api/expedientessecundarios/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['expedientesSecundarios'] });
