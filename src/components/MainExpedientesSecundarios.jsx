...
const MainExpedientesSecundarios = ({ className }) => {
    ...
    const mutationSaveExpediente = useMutation({
        mutationFn: async (data) => {
            ...
            const url = data.id ? `http://143.131.204.234:9000/api/expedientessecundarios/${data.id}` : 'http://143.131.204.234:9000/api/expedientessecundarios';
    ...
    const mutationDeleteExpediente = useMutation({
        mutationFn: (id) =>
            fetch(`http://143.131.204.234:9000/api/expedientessecundarios/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['expedientesSecundarios'] });
