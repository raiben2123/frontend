...
const MainExpedientesSecundarios = ({ className }) => {
    ...
    const mutationSaveExpediente = useMutation({
        mutationFn: async (data) => {
            ...
            const url = data.id ? `http://localhost:8081/api/expedientessecundarios/${data.id}` : 'http://localhost:8081/api/expedientessecundarios';
    ...
    const mutationDeleteExpediente = useMutation({
        mutationFn: (id) =>
            fetch(`http://localhost:8081/api/expedientessecundarios/${id}`, {
                method: 'DELETE',
            }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['expedientesSecundarios'] });
